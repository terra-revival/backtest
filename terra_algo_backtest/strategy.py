...
class DivStrategy(Strategy):
    def __init__(
        self, cp_amm: ConstantProductEngine, args: dict, arb_enabled: bool = False
    ):
        self.args = args
        self.cp_amm = cp_amm
        self.arb_strategy = ArbStrategy(cp_amm) if arb_enabled else None
    ...
    def execute_div(
        ...
        trade_exec_info = []
        self.cp_amm.update_mkt_price(mkt_price)
        current_row = {"trade_date": dt}
        executed = self.cp_amm.execute_trade(current_row, trade_order)
        executed["arb_profit"] = 0
        executed["peg"] = self.args["soft_peg_price"]
        if is_buy_back:
            # buy back is in USDT, so convert to USTC
            executed["buy_back_vol"] = trade_order.order_size / executed["price"]
        else:
            cex_profit, chain_profit = self.calc_taxes(
                executed["price"],
                trade_order.order_size
                if trade_order.direction == "buy"
                else -trade_order.order_size,
            )
    ...
    def calc_taxes(self, exec_price: float, volume: float) -> Tuple[float, float]:
        ...
        if exec_price < self.args["soft_peg_price"] or (
            ...
            T = abs(
                (exec_price - self.args["soft_peg_price"])
                * (1 - self.args["arbitrage_coef"])
                if self.args["soft_peg_price"] == 1
                else 1
            )
            volume = abs(volume if volume > 0 else volume * exec_price)
            # we multiply by 2, so we get to 100% tax
            # at a 50% price difference and stick to 100% max
            tax_percentage = (
                2 * T / self.args["soft_peg_price"]
                if 2 * T / self.args["soft_peg_price"] < 1
                else 1
            )
            tax = tax_percentage * abs(volume)
            return round(tax * self.args["cex_tax_coef"], 5), round(
                tax * (1 - self.args["cex_tax_coef"]), 5
            )
        else:
            return 0, 0
    ...
    def time_peg_increase(self, function: List[float], idx: int) -> bool:
        ...
        if self.args["soft_peg_price"] >= 1:
            return False
        num_steps = (1 - self.args["soft_peg_start"]) / self.args[
            "soft_peg_increase"
        ] + 1
        index_delta = len(function) / num_steps
        last_change = (
            self.args["last_peg_change"] if "last_peg_change" in self.args else 0
        )
    ...
    def avg_price_peg_increase(
        self, function: List[float], idx: int, trade_history: List[dict]
    ) -> bool:
        # if we are at peg of 1, we dont increase anymore
        if self.args["soft_peg_price"] >= 1:
            return False
