...
from terra_algo_backtest.strategy import div_protocol
...
# get trades from csv
df_ustc = pd.read_csv("terra_algo_backtest/datasets/luna_kraken.csv", parse_dates=[
                      "date"], index_col="date")
sims['kraken_ustc/usdt_m1'] = df_ustc['close']
...
div_protocol_args = {
    ...
    "arbitrage_coef": 0.2,
    "cex_tax_coef": 0.4,  # ratio of how much tax CEX gets
    # not used yet, we are not clear if to buyback with all the tax or keep base token also. 1 = spend all base tokens
    "buy_backs_coef": 1,
    "timeframe": 3600 * 4,  # time difference between 2 price points of inputs in seconds, makes up axis X on charts. tied to steps in simulationSamples
...
for sim in sims:
    ...
    simul = peg_simulation(use_market, sims[sim], div_protocol, div_protocol_args)
    simul['sim_name'] = sim
    # display results
    show(new_simulation_figure(use_market, simul))
